...
export class SupplierEditComponent extends DefaultComponentBase
	implements OnInit, IUiAction<CM_SUPPLIER_ENTITY>, AfterViewInit {
	constructor(
		injector: Injector,
				...
				this.supplierService
					...
					.subscribe((response) => {
						if (response.result != '0') {
							...
							this.addNewSuccess();
							this.inputModel.suP_CODE = response.id;
							...
							if (!this.isApproveFunct) {
								...
								this.supplierService
									.cM_SUPPLIER_App(response.id, this.appSession.user.userName)
									.pipe(
										finalize(() => {
											this.saving = false;
										})
	...
	goBack() {
		this.navigatePassParam('/app/admin/supplier', null, undefined);
	}
		...
		this.message.confirm(
			...
			(isConfirmed) => {
				if (isConfirmed) {
					this.saving = true;
					this.supplierService
						.cM_SUPPLIER_App(this.inputModel.suP_ID, currentUserName)
						.pipe(
							finalize(() => {
								this.saving = false;
							})
