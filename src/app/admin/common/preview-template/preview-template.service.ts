...
import { ReportTable, AppMenuServiceProxy, AppMenuDto, API_BASE_URL, GetRoleForEditOutput, FlatPermissionDto, AsposeServiceProxy as ReportServiceProxy, ReportParameter, ReportTemplateServiceProxy, CM_REPORT_TEMPLATE_ENTITY, ReportInfo, ReportHtmlInfo } from '@shared/service-proxies/service-proxies';
...
export class PreviewTemplateService extends EvalFunctionComponent {
    ...
    private QRCodeGenerator(template: string): string {
        let data = [];
        var TenNhan=template.substr(0,28);
        //var GT =new String(template.substr(208,26).toString);
        ...
        while (template.match(regexEval)) {
            ...
            data.push({name: qrContent, data: this.genQRCode(qrContent,'image')});

            //console.log(TenNhan)
            if(TenNhan=="<p hidden>van_phong_size_4</")
            {
                if (image) {
                    var qrImage = "<img style='width:77.6px; height: 77.6px;' class='qrcode' alt='QR Code' src='" + image + "'/>"
            ...
            if(TenNhan=="<p hidden>nha_may_size_1</p>")
            {
                if (image) {
                    var qrImage = "<img style='width:132.3px; height: 132.3px;' class='qrcode' alt='QR Code' src='" + image + "'/>"
                    template = template.split(qrString).join(qrImage);
                }
                else {
                    template = template.split(qrString).join("");
            ...
            if(TenNhan=="<p hidden>nha_may_size_3</p>")
            {
                if (image) {
                    var qrImage = "<img style='width:86.9px; height: 86.9px;' class='qrcode' alt='QR Code' src='" + image + "'/>"
                    template = template.split(qrString).join(qrImage);
                }
                else {
                    template = template.split(qrString).join("");
            ...
            {
                if (image) {
                    var qrImage = "<img style='width:94.5px; height: 94.5px;' class='qrcode' alt='QR Code' src='" + image + "'/>"
                    template = template.split(qrString).join(qrImage);
                }
                else {
                    template = template.split(qrString).join("");
        ...
        sessionStorage.setItem( 'dataImage', JSON.stringify(data));
        return template;
    }

    //TienLee 12/03/22
    private QRCodeGenerator_SaveFolder(template: string) {
        let data = [];
        var TenNhan=template.substr(0,28);
        //var GT =new String(template.substr(208,26).toString);
        var regexEval = new RegExp(ReportKeyConsts.KeyQRCode + '\\{(.*?)\\}', 'g');
        while (template.match(regexEval)) {
            ...
            //console.log(qrContent)
            var image = this.genQRCode(qrContent)
            data.push({name: qrContent, data: image});
        }
        this.downloadFolder(data)
    }
    private downloadFolder(data: any[]){
        const jszip = new JSZip();
        for(let i = 0; i < data.length; i++) {
          var binary = atob(data[i]['data'].split(',')[1]);
          var array = [];
          ...
          jszip.file(`QRCode/${data[i]['name']}.png`, image)
    ...
    print(data: string) {
        ...
        newWindow.document.write('<html><head><meta name="viewport" content="width=device-width"></head><body onload="window.print()">' + data + '</body></html>');
    ...
    printContent(content: string, data: ReportTable[], done = undefined) {
        ...
        this.ReloadDataFromStoreToPreview(content, data).then(result => {
            ...
            //luatndv test xuat theo dinh dang pdf
            var reportInfo = new ReportHtmlInfo();
            // reportInfo.parammeters = [];
            // reportInfo.values = [];

                reportInfo.typeExport = "p";
    ...
    getFileEPS(filE_NAME: string, params: ReportParameter[], values: ReportParameter[]) {
        abp.ui.setBusy(null, this.l('SavingWithThreeDot'));
            var reportInfo = new ReportInfo();
            reportInfo.storeName = ''
            reportInfo.parameters = params;
            reportInfo.values = values;
            this._reportService.getFileFromStore(reportInfo).subscribe(storeData => {
                console.log(storeData)
               
                  var  contentType = contentType || '';
                  var  sliceSize = sliceSize || 512; // sliceSize represent the bytes to be process in each batch(loop), 512 bytes seems to be the ideal slice size for the performance wise 
            
                    var byteCharacters = atob(storeData);
                    var byteArrays = [];
