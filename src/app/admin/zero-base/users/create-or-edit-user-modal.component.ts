...
import { CreateOrUpdateUserInput, OrganizationUnitDto, PasswordComplexitySetting, ProfileServiceProxy, UserEditDto, UserRoleDto, UserServiceProxy, DepartmentServiceProxy, CM_DEPARTMENT_ENTITY, TokenAuthServiceProxy, CM_BRANCH_ENTITY, BranchServiceProxy, AccountServiceProxy, TlUserServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditUserModalComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        private _sendMailConfirm: AccountServiceProxy,
        private _check: TlUserServiceProxy,
    ...
    show(userId?: number): void {
        ...
        this._userService.getUserForEdit(userId).subscribe(userResult => {
            ...
            // if (this.user.isEmailConfirmed == true){
    ...
    save(): void {
        ...
        // this._userService.getUserForEdit(this.user.id).subscribe(userResult => {
        //     let u = userResult.user;
        //     if(u.isEmailConfirmed == false) {
        //         this.message.info(this.l('NotActivationMail'), this.l('EmailNoti'))
        //         return;
        //     }
        //     else if(u.emailAddress != this.user.emailAddress && u.isEmailConfirmed == true) {
        //         this.message.info(this.l('YouHaveBeenChangeMail'), this.l('PleaseActiveEmail'))
        //         return;
        //     }
        //     else {
        //         abp.ui.setBusy();
    ...
    emailChange() {
        this._userService.getUserForEdit(this.user.id).subscribe(userResult => {
            let u = userResult.user;
                if (u.emailAddress == this.user.emailAddress && u.isEmailConfirmed == true){
                    this.checkClick = true;
                    this.updateView();
                }
                else {
