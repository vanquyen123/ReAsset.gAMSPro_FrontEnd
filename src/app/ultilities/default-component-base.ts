...
export abstract class DefaultComponentBase extends ChangeDetectionComponent{
    ...
    @ViewChild('appToolbar') appToolbar: ToolbarRejectExtComponent;
    ...
    approveImportWareFSuccess() {
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.ApproveImportWareF;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonApproveEnable(false);
        this.updateView();
    ...
    approveKSSSuccess() {
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.Confirm;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonApproveKSSEnable(false);
        this.updateView();
    }
    approveDeputySuccess() {
        this.showSuccessMessage(this.l('SuccessfullyApprove'));
        let routerKey = this.activeRoute['_routerState'].snapshot.url;
        routerKey = routerKey.substr(0, routerKey.indexOf('-view;'));
        if (this.cacheRouteReuseStrategy.storedRouteHandles.get(routerKey)) {
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.ApproveDeputy;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonApproveDeputyEnable(false);
        this.updateView();
    }
    approveKhoiSuccess() {
        this.showSuccessMessage(this.l('SuccessfullyApprove'));
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.ApproveKhoi;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonApproveDeputyEnable(false);
        this.updateView();
    ...
    accessSuccess() {
        this.showSuccessMessage(this.l('SuccessfullyAccess'));
        let routerKey = this.activeRoute['_routerState'].snapshot.url;
        routerKey = routerKey.substr(0, routerKey.indexOf('-view;'));
        if (this.cacheRouteReuseStrategy.storedRouteHandles.get(routerKey)) {
            this.cacheRouteReuseStrategy.storedRouteHandles.get(routerKey)['componentRef'].instance['shouldResetTable'] = true;
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.Access;
        }
        abp.ui.clearBusy();
    ...
    finishCheckSuccess() {
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.FinishCheck;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonFinishCheckEnable(false);
        this.updateView();
    }
    accessStorekeepersSuccess() {
        this.showSuccessMessage(this.l('SuccessfullyAccess'));
        let routerKey = this.activeRoute['_routerState'].snapshot.url;
        routerKey = routerKey.substr(0, routerKey.indexOf('-view;'));
        if (this.cacheRouteReuseStrategy.storedRouteHandles.get(routerKey)) {
        ...
        if (this['inputModel']) {
            this['inputModel'].autH_STATUS = AuthStatusConsts.AccessStorekeepers;
        }
        abp.ui.clearBusy();
        this.appToolbar.setButtonAccessStorekeepersEnable(false);
        this.updateView();
    }
    rejectSuccess() {
