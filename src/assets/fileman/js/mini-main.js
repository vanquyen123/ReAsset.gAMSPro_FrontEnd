...
var filemanShowCheckbox = true;
var filemanfilesChecked = [];
...
RoxyUtils.FileExists = function(path) {
    ...
    var tokenAuth = getCookie('Abp.AuthToken');
    var customHeaders = {
...
RoxyUtils.LoadConfig = function() {
    ...
    $.ajax({
        url: '/assets/fileman/conf-server.json',
        dataType: 'json',
        async: false,
        success: function(data) {

            $.ajax({
                url: '/assets/' + data.SERVER_PREFIX,
                dataType: 'json',
                async: false,
                success: function(json) {
                    serverPrefix = json.remoteServiceBaseUrl;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            RoxyFilemanConf.SERVER_PREFIX = serverPrefix;
            RoxyFilemanConf.DIRLIST = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.DIRLIST;
            RoxyFilemanConf.CREATEDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.CREATEDIR;
            RoxyFilemanConf.DELETEDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.DELETEDIR;
            RoxyFilemanConf.MOVEDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.MOVEDIR;
            RoxyFilemanConf.COPYDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.COPYDIR;
            RoxyFilemanConf.RENAMEDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.RENAMEDIR;
            RoxyFilemanConf.FILESLIST = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.FILESLIST;
            RoxyFilemanConf.UPLOAD = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.UPLOAD;
            RoxyFilemanConf.DOWNLOAD = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.DOWNLOAD;
            RoxyFilemanConf.DOWNLOADDIR = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.DOWNLOADDIR;
            RoxyFilemanConf.DELETEFILE = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.DELETEFILE;
            RoxyFilemanConf.MOVEFILE = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.MOVEFILE;
            RoxyFilemanConf.COPYFILE = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.COPYFILE;
            RoxyFilemanConf.RENAMEFILE = RoxyFilemanConf.SERVER_PREFIX + RoxyFilemanConf.RENAMEFILE;
        },
        error: function(data) {
...
RoxyUtils.LoadLang = function() {
    ...
    if (RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto') {
        if (RoxyUtils.GetUrlParam('langCode')) {
            langUrl = RoxyFilemanConf.SERVER_PREFIX + '/fileman/lang/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json';
        } else {
            var language = window.navigator.userLanguage || window.navigator.language;
            langUrl = RoxyFilemanConf.SERVER_PREFIX + '/fileman/lang/' + language.substr(0, 2) + '.json';
        }
        if (!RoxyUtils.FileExists(langUrl))
            langUrl = '';
    } else {
        if (RoxyFilemanConf.LANG) {
            langUrl = RoxyFilemanConf.SERVER_PREFIX + '/fileman/lang/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json';
            if (!RoxyUtils.FileExists(langUrl))
                langUrl = '';
        }
    }
...
// filemanShowCheckbox begin

function oncheckfile(elem) {
    var path = $(elem).closest('li').attr('data-path');
    if (elem.checked) {
        filemanfilesChecked.push(path);
    } else {
        filemanfilesChecked = filemanfilesChecked.filter(x => x != path);
    }
}

// filemanShowCheckbox end
...
function File(filePath, fileSize, modTime, w, h) {
    ...
    this.height = (h ? h : 0);
    this.Show = function() {
        html = '<li data-path="' + this.fullPath + '" data-time="' + this.time + '" data-icon="' + this.icon + '" data-w="' + this.width + '" data-h="' + this.height + '" data-size="' + this.size + '" data-icon-big="' + RoxyFilemanConf.SERVER_PREFIX + (this.IsImage() ? this.fullPath : this.bigIcon) + '" title="' + this.name + '">';
        if (filemanShowCheckbox) {
            if (filemanfilesChecked.indexOf(this.fullPath) >= 0) {
                html += "<div class='chk'> <input checked onchange='oncheckfile(this)' type='checkbox'></div>"
            } else {
                html += "<div class='chk'> <input onchange='oncheckfile(this)' type='checkbox'></div>"
            }
        }

        html += '<img src="' + RoxyFilemanConf.SERVER_PREFIX + this.icon + '" class="icon">';
    ...
    this.Delete = function() {
        ...
        var deletePath = this.fullPath;
        if (deletePath.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
            deletePath = deletePath.slice(RoxyFilemanConf.SERVER_PREFIX.length);
        }
        var deleteUrl = RoxyUtils.AddParam(RoxyFilemanConf.DELETEFILE, 'f', deletePath);
        var item = this;
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.res.toLowerCase() == 'ok') {
                    $('li[data-path="' + deletePath + '"]').remove();
                    var folderPath = item.path;
                    if (folderPath.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
                        folderPath = folderPath.slice(RoxyFilemanConf.SERVER_PREFIX.length);
                        if (folderPath.indexOf('/') != 0) {
                            folderPath = '/' + folderPath;
                        }
                    }
                    var d = Directory.Parse(folderPath);
                    if (d) {
                        d.files--;
                        d.Update();
                        d.SetStatusBar();
    ...
    this.Rename = function(newName) {
        var filePath = this.fullPath;
        if (filePath.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
            filePath = filePath.slice(RoxyFilemanConf.SERVER_PREFIX.length);
        }
        ...
        if (!newName)
            return false;
        var url = RoxyUtils.AddParam(RoxyFilemanConf.RENAMEFILE, 'f', filePath);
        ...
        if (path.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
            path = path.slice(RoxyFilemanConf.SERVER_PREFIX.length);
            if (path.indexOf('/') != 0) {
                path = '/' + path;
            }
        }
        ...
        $.ajax({
            ...
            type: 'POST',
            headers: customHeaders,
            data: { f: filePath, n: newName },
            dataType: 'json',
            async: false,
            success: function(data) {
                if (data.res.toLowerCase() == 'ok') {
                    var newPath = RoxyUtils.MakePath(path, newName);
                    ...
                    $('li[data-path="' + filePath + '"] .icon').attr('src', RoxyUtils.GetFileIcon(newName));
                    $('li[data-path="' + filePath + '"] .name').text(newName);
                    $('li[data-path="' + filePath + '"]').attr('data-path', newPath);
                    ret = true;
                }
                if (data.msg)
                    alert(data.msg);
    ...
    this.Move = function(newPath) {
        ...
        var fp = filePath;
        if (fp.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
            fp = fp.slice(RoxyFilemanConf.SERVER_PREFIX.length);
        }

        var ifp = item.fullPath;

        if (ifp.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
            ifp = ifp.slice(RoxyFilemanConf.SERVER_PREFIX.length);
        }
        ...
        $.ajax({
            ...
            type: 'POST',
            headers: customHeaders,
            data: { f: fp, n: newFullPath },
            dataType: 'json',
            async: false,
            success: function(data) {
                if (data.res.toLowerCase() == 'ok') {
                    $('li[data-path="' + ifp + '"]').remove();
...
function Directory(fullPath, numDirs, numFiles) {
    ...
    this.LoadAll = function(selectedDir) {
        ...
        var t = document.getElementById('roxyCustomPanel');
        var attr = window.frameElement.attributes['folder-upload'];
        var folderUpload = undefined;
        if (attr) {
            folderUpload = attr.value;
        }
        if (folderUpload) {
            dirListURL += '&functionFolder=' + folderUpload;
        }
        var isMultiple = '0';

        // var isMultipleAttr = window.frameElement.attributes['is-multiple'];
        // if (isMultipleAttr) {
        //     isMultiple = isMultipleAttr.value;
        // }
        // filemanfilesChecked = [];

        // if (isMultiple == '1') {
        //     filemanShowCheckbox = true;
        // } else {
        //     filemanShowCheckbox = false;
        // }
        ...
        $.ajax({
            ...
            success: function(dirs) {
                ...
                if (!selectedDir && folderUpload) {
                    selectedDir = dirs[0].p + '/' + folderUpload;
                }
...
window.parent.refreshFilemanWindow = function(isMultiple) {
    filemanShowCheckbox = isMultiple;
    var path = $('#pnlDirList>li>ul>li>.ui-droppable.selected').closest('li').attr('data-path');
    var d = new Directory(path);
    filemanfilesChecked = [];
    d.LoadAll(path);
}
...
function fileUpload(f, i) {
    ...
    http.open("POST", RoxyFilemanConf.UPLOAD);
...
function getSelectedFile() {
    ...
    if ($('#pnlFileList .selected').length > 0)
        ret = new File(RoxyFilemanConf.SERVER_PREFIX + $('#pnlFileList .selected').attr('data-path'));
    return ret;
}
...
function downloadFile() {
    var f = getSelectedFile();
    var tokenAuth = getCookie('Abp.AuthToken');
    var pathDownload = f.fullPath;
    if (pathDownload.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
        pathDownload = pathDownload.slice(RoxyFilemanConf.SERVER_PREFIX.length);
    }
    if (f && RoxyFilemanConf.DOWNLOAD) {
        var url = RoxyUtils.AddParam(RoxyFilemanConf.DOWNLOAD, 'f', pathDownload);
...
function tooltipContent() {
    ...
    if ($('#hdViewType').val() == 'thumb' && f.IsImage()) {
        ...
        if (RoxyFilemanConf.GENERATETHUMB) {
            ...
            imgUrl = f.fullPath;
        html = '<img src="' + RoxyFilemanConf.SERVER_PREFIX + imgUrl + '" class="imgPreview"><br>' + f.name + ' <br><span class="filesize">' + t('Size') + ': ' + RoxyUtils.FormatFileSize(f.size) + ' ' + t('Dimensions') + ': ' + f.width + 'x' + f.height + '</span>';
    } else
...
function pasteToFiles(e, el) {
    ...
    if (d && clipBoard && clipBoard.obj) {
        if (clipBoard.action == 'copy')
            clipBoard.obj.Copy(filePath);
        else {
            clipBoard.obj.Move(filePath);
            clearClipboard();
        }
    }
    return true;
...
function setFile() {
    ...
    if (insertPath.indexOf(RoxyFilemanConf.SERVER_PREFIX) == 0) {
        insertPath = insertPath.slice(RoxyFilemanConf.SERVER_PREFIX.length);
    }
    if (RoxyFilemanConf.RETURN_URL_PREFIX) {
        var prefix = RoxyFilemanConf.RETURN_URL_PREFIX;
        if (prefix.substr(-1) == '/')
    ...
    switch (getFilemanIntegration()) {
        case 'ckeditor':
            window.opener.CKEDITOR.tools.callFunction(RoxyUtils.GetUrlParam('CKEditorFuncNum'), RoxyFilemanConf.SERVER_PREFIX + insertPath);
            self.close();
            break;
        case 'tinymce3':
            var win = tinyMCEPopup.getWindowArg("window");
        ...
        default:
            ...
            ff.fullPath = insertPath;
            if (filemanShowCheckbox) {
                FileSelectedMulti(filemanfilesChecked);
            } else {
                FileSelected(ff);
            }
            break;
